...
namespace ReadIt.Extentions.ImageExtention
{
    public class ImageExtension<T> : IImageExtension<T> where T : class
    {
        ...
        public void DeleteImages(long id)
        {
            ...
            {
                oldMediaPath = Path.Combine(Constants.MediaPath, "Media", folder, mediaName);
            }
            // delete the previous images from the server's directory
            if (oldMediaPath != null)
            {
                foreach (var file in Directory.GetFiles(Path.Combine(Constants.MediaPath, "Media", folder)))
                {
        ...
        public void AddImages(IFormFile file, T tbEntity)
        {
            ...
            var filePath = Path.Combine(Constants.MediaPath, "Media", folder, fileName);
        ...
        public string GetImage(T tbEntity)
        {
            ...
            if (entityType == "TbBlog")
            {
                ...
                if (media != null)
                {
                    var filePath = Path.Combine(Constants.MediaPath, "Media", "Blog Images", media.MediaPath);
            ...
            else if (entityType == "TbUser")
            {
                ...
                {
                    var filePath = Path.Combine(Constants.MediaPath, "Media", "Profile Images", (tbEntity as TbUser).Avatar);
                    byte[] fileBytes = File.ReadAllBytes(filePath);
                    string base64String = Convert.ToBase64String(fileBytes);
                    image = base64String;
                }
